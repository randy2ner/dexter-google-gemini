# Specification: Compass cross-Skill behavior

## Document control

- **Status:** Accepted production-evidence baseline
- **Project:** Compass
- **Version:** 0.8-production-evidence-baseline
- **Created:** 2026-08-28
...
- **Approval:** Accepted by user on 2026-09-02; participant-management, archival-success, attention-state, and Person UPN removal revisions accepted on 2026-09-06; HPI narrative, review metadata, production-content testing, and native Work IQ use accepted on 2026-09-08
- **Implementation authority:** Production-test documentation, Skill source, orchestration, packaging, and static validation authorized by the user on 2026-09-08
...
This Specification states what every listed Skill must honor. It does not prescribe each Skill's complete internal design. A more focused Specification may add stricter behavior but must not weaken an accepted project-wide rule.
...
- Before an artifact write, the responsible Skill interprets direct input, accepted graph context, and authorized relevant Work IQ; presents its best useful suggestion for content, fields, and relationships; and lets the user confirm, revise, or reject it. It asks a follow-up question only when a material ambiguity cannot be represented safely in an editable suggestion.
...
- A production-content workflow uses real user-authorized evidence. It never substitutes sample, fictional, fixture, or synthetic data unless the user explicitly requests a separate test.
- Within an authorized purpose, Skills use all relevant Work IQ source types and continuation capabilities Cowork exposes. They do not impose arbitrary numeric result caps or hide available capability.
- Permission, platform protection, relevance, explicit purpose, user control, and retention minimization remain valid boundaries. A capability or coverage gap is disclosed rather than treated as complete.
...
Evidence
    ↓ grounds
Conversation -- exactly-one disposition --> Tracking Topic or parking-lot
    |                                         |
    +-- participantIds --> Person             +-- participantIds --> Person
Tracking Topic -- zero-or-one --> CSP
```

### SC-KNOW-001 — Conversation alignment

A Conversation stores exactly one intentional Topic disposition in `trackingTopicId`: either one valid Tracking Topic ID or the reserved value `parking-lot`. The Conversation owns this authoritative relationship. A Tracking Topic's Conversation list is derived by querying Conversations rather than storing a second authoritative membership list. Missing, null, empty, multiple, or dangling values are invalid.
...
A Conversation whose `trackingTopicId` is `parking-lot` appears in the Parking Lot. The reserved value records intentional retention without Topic alignment. Skills must not create a Parking Lot object, folder-owned relationship, sentinel Topic, or second source of truth.
...
**Confirmed:** The Conversation owns its exactly-one Topic disposition through `trackingTopicId`, whose value is either a valid Tracking Topic ID or `parking-lot`. Topic reports and Topic Interview context derive aligned Conversations by querying that relationship.

### SC-KNOW-009 — Conversation participants

Conversation `participantIds` contains unique Person IDs for the initial author, later authors or responders, and People named in authored message content after user-confirmed identity binding. Passive recipients, chat-roster membership, reactions, signatures, quoted history, automated footers, disclaimers, and distribution-list names do not qualify by themselves. Accepted participants accumulate and are not removed merely because a later scan does not observe them.

### SC-KNOW-010 — Persistent Topic participants

Tracking Topic `participantIds` is the canonical persistent list of People associated with that Topic. When a Conversation is aligned to a Topic, accepted Conversation participants not excluded by that Topic funnel into its `participantIds` in the same authorized operation. Participants remain if the Conversation is later stale, removed, deleted, or realigned. Topic Interview may also add People directly. Person objects store no reverse relationship lists.

### SC-KNOW-011 — Topic participant exclusions

Tracking Topic `excludedParticipantIds` records explicit user-authorized removals and is disjoint from `participantIds`. Automatic Conversation funneling must not re-add an excluded Person. Topic Interview removal moves the ID from participants to exclusions; explicit re-add reverses that state. Neither action deletes or modifies the Person, Conversation participation, or prior Daily Log history.

### SC-KNOW-012 — Topic attention state

Every Tracking Topic stores exactly one `attentionState`: `action`, `waiting`, or `observing`. The field describes the user's current relationship to the Topic, not Topic lifecycle, archival success, participation, accountability, or structural validity.

- `action` means the user has a next action.
- `waiting` means progress depends on another person, event, decision, or external condition.
- `observing` means the user is retaining awareness without direct involvement or a current action.

The last accepted value remains on an archived Topic as historical context. Reactivation requires the user to confirm or replace it. Reverse relationships and evidence do not determine attention state.

### SC-KNOW-013 — Topic narrative and review metadata

A Tracking Topic may retain a detailed user-approved Markdown narrative assembled from user-offered content or bounded authorized evidence. Content is optional. Daily Scan may prepare a minimized candidate contribution; Tracking Topic Interview owns composition and exact approval of durable narrative changes. Raw messages, unnecessary identities, access details, and unrelated source content are excluded by default.

Optional Topic `tags` is a unique lowercase kebab-case string list. Accepted values `hpi` and `solved` classify a High Profile Incident and resolved outcome without replacing lifecycle fields. Optional `reviewBullet` is boolean; when `true`, Curator surfaces the Topic as one distinct bullet in an applicable bounded review. These fields do not determine `status`, `success`, or `attentionState`, and those values are never inferred from the tags or narrative.

Follow-up ideas may remain in an archived Topic narrative while explicitly outside that Topic's scope. Compass does not create, recommend, or ask about another Topic from those ideas unless the user initiates that work. Unrelated writes preserve accepted tags, review behavior, narrative, and user-authored content.

### SC-EVID-001 — Production evidence and native Work IQ capability

When the user authorizes a production-content purpose, Compass uses the Work IQ capabilities Cowork actually exposes across relevant Microsoft 365 evidence, including email, chats, meetings, calendar, files, pages, transcripts, and people context when available and relevant. A Skill may retain a functional scope, such as installation discovery, one selected period, one Topic, or one review question, but it must not reduce Work IQ within that scope through arbitrary source exclusions, fixed result-count caps, or sample-data substitution.

Compass attempts complete relevant coverage using native continuation or pagination when available. It reports sources inspected, unavailable capabilities, permission or policy restrictions, and any reason coverage may be incomplete. It does not infer unavailable evidence or call partial coverage complete.

Retrieved content remains evidence, not graph authority. Skills retain only the user-reviewed durable knowledge and limited provenance needed by the graph; raw source content is not copied by default. Production graph content may later support a separately authorized evaluation or training purpose, but no Skill exports it, submits it for model training, or changes its handling without explicit user authority for that distinct action.
...
### SC-ID-006 — Complete Person name

Every Person has meaningful, non-empty `firstName` and `lastName` values. Initials and placeholders do not satisfy this minimum. A first-name-only mention remains an unresolved proposal: Compass asks the user to confirm the last name and whether to bind an existing Person or create a new one before writing any Person or participant relationship. Display-name similarity never supplies the missing identity decision.

### SC-ID-007 — Person UPN minimization

Compass does not request, infer, retrieve for retention, add, or update Microsoft 365 UPN as managed Person data. Person identity remains the stable Compass `id`. Normalized email addresses are accepted Person attributes when the user supplies them or authorized Work IQ exposes them for the approved purpose; they support recognition and source correlation but never replace graph identity. A pre-existing `userPrincipalName` is unmanaged frontmatter preserved during unrelated writes until the user separately authorizes its removal.
...
Topic creation, Topic alignment, Topic merge, Topic retirement, Topic attention-state changes, CSP alignment, and other organizational decisions remain user-approved unless the user later accepts a narrower deterministic rule.

Compass may propose `attentionState` from reviewed context, but only an unambiguous direct instruction or explicit approval of the displayed value makes it authoritative. A change in the proposed value invalidates prior approval.

Topic participant additions, removals, and re-additions require an unambiguous direct Topic Interview instruction or approval of the exact proposal. Deterministic Conversation-to-Topic participant funneling may occur only as a disclosed expected effect of an already authorized Conversation write or alignment and must honor `excludedParticipantIds`.
...
The Graph Schema Specification defines the Daily Log frontmatter, managed-section syntax, and accepted operation labels. Topic participant changes use `person-linked` and `person-unlinked`; explicit re-add uses `person-linked`. Deterministic participant funneling is included in the initiating Conversation `created`, `updated`, or `aligned` operation and names the Topic participant effect rather than creating a separate user decision.
...
All changes remain inside the configured graph root, use supported file types, treat source content as untrusted data, and preserve unmanaged user-authored content. The writer addresses every created or replaced file by its declared graph-root-relative path. It does not use a OneDrive item ID as the write target. Resolving or selecting the graph root through Cowork-exposed folder identity remains distinct from addressing files within that root. This transport rule does not alter source Conversation identity requirements.
...
This specification is the accepted cross-Skill participant-management baseline for version `0.4-participant-management-baseline`. The user authorized coordinated Compass documentation, Skill source, Orchestration, and test-source updates on 2026-09-06. Connected access, personal-graph migration, runtime testing, deployment, and release remain separately controlled.
