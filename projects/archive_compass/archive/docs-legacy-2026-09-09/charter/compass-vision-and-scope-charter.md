...
Compass uses the user's request, accepted graph context, and authorized Work IQ understanding to suggest what a useful artifact should contain. It presents that interpretation for confirmation or revision rather than requiring the user to translate their work into schema fields. A suggestion remains non-authoritative until the user confirms it.
...
11. **Interpret, suggest, confirm:** Use work context to propose useful artifact content in ordinary language, then let the user confirm or revise it before writing.
