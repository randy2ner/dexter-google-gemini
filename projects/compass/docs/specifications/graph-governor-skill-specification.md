# Graph Governor Skill Specification
...
- **Version:** 0.3-dogfood-responsibility
- **Created:** 2026-08-28
...
3. the accepted [Compass Graph Schema Specification 0.3](compass-graph-schema-specification.md).
...
- Each Conversation has zero or one authoritative Tracking Topic relationship stored on the Conversation.
...
The first Beta validates, reports, and guides. It does not independently originate a graph modification. During an explicitly authorized disposable synthetic test, it may supervise the accepted recovery protocol without gaining authority to choose or change semantic meaning.
...
- supervise the accepted disposable-beta write and recovery protocol for an authorized test; and
...
- use the disposable-beta recovery protocol on a personal or production graph; or
- claim the whole graph is healthy when only a bounded scope was inspected.

Within the disposable synthetic protocol, restoration is allowed only under exact accepted preconditions and test authority. Connected dogfood state never uses automatic deletion-based rollback. A partial or unverifiable connected application returns `recovery-required`, accounts for every effect, and blocks dependent writes pending user-led resolution.
...
A stale active Topic may be proposed for archival, and an archived Topic may be reactivated, but Graph Governor rejects every operation that would delete a Tracking Topic.
...
6. What production persistence and recovery protocol should replace the synthetic-only protocol after beta evidence?

## 16. Acceptance boundary

This specification is part of the accepted five-Skill Slice B set and defines Graph Governor's beta responsibility boundary and compatibility with Shared Contracts and Graph Schema version `0.3-beta-baseline`.
