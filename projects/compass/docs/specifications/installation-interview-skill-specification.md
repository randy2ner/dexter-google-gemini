# Specification: Installation Interview Skill responsibility

## Document control

- **Status:** Accepted responsibility specification
- **Version:** 0.2.3-dogfood-responsibility
...
| INST-002 | Propose foundational CSP, Tracking Topic, and Person objects only from direct user input or separately reviewed patterns. | `PR-AUTH-001`, `PR-PRIV-001` | Every created object is traceable to displayed user authority. |
| INST-003 | Preserve canonical relationship ownership and accepted object fields. | `PR-PORT-001`, `PR-GRAPH-001` | Topics own `cspId`; no reverse authoritative lists are created. |
...
| INST-010 | After selected-source retrieval, prove representative plain-text write, read-back, and cleanup at the confirmed graph root before creating managed structure. | `PR-SAFE-001`, `PR-TRUTH-001` | Only an actual refusal returns `write-blocked` / `blocked`; no configuration, Daily Log, managed subfolder, or object is created. |
| INST-011 | Apply an approved bootstrap config-first. | `PR-SAFE-001`, `PR-TRUTH-001` | `_compass/config.yaml` is written and verified before remaining managed folders and objects are created. |
...
- An observed DLP, sensitivity-label, or protection-policy refusal of representative plain text produces setup classification `write-blocked` and terminal outcome `blocked`, not `failed`. Classification alone does not produce that outcome.
- A blocked result reports the platform reason, graph root, probe cleanup state, zero configuration and Daily Log writes, and whether only an empty user-confirmed root may remain.
...
- Standalone Activity objects are not part of schema version 1; installation activity is represented in the Daily Log.
